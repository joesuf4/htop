...
static void ZfsCompressedArcMeter_display(Object* cast, RichString* out) {
   ...
   if (this->values[0] > 0) {
      ...
      RichString_append(out, CRT_colors[COLOR_METER_TEXT], " Uncompressed, ");
      Meter_humanUnit(buffer, this->values[0], 50);
      RichString_append(out, CRT_colors[COLOR_METER_VALUE], buffer);
      RichString_append(out, CRT_colors[COLOR_METER_TEXT], " Compressed, ");
      ZfsCompressedArcMeter_printRatioString(this, buffer, 50);
      RichString_append(out, CRT_colors[COLOR_METER_VALUE], buffer);
      RichString_append(out, CRT_colors[COLOR_METER_TEXT], " Ratio");
   } else {
      RichString_write(out, CRT_colors[COLOR_METER_TEXT], " ");
      RichString_append(out, CRT_colors[COLOR_FAILED_SEARCH], "Compression Unavailable");
   }
}
